...
def find_nearby_factors_int(X, Y, Z, search_range=10):
    ...
    # If we didn't find exact factors, throw an error
...
def cosim_mm1(minx: float, maxx: float, miny: float, maxy: float, res: int, sim_num: int, k: int, rad: int):
    ...
    #Create GUID
    guid = str(uuid.uuid4())
    output_base_dir = os.path.join(SITE_ROOT, "output") # TODO: Make Unique to inbound request (to avoid conflicts on concurrent requests)
    guid_dir = os.path.join(output_base_dir,guid)
    
    
    for current_sim in range(sim_num):
        logPrint(f"Starting Request: {guid} Simulation: {current_sim} ...")
        output_dir = os.path.join(guid_dir,str(current_sim))
...
#           Y: From -1800000 to -1,650000
        ...
        k = k         # number of neighboring data points used to estimate a given point 
        rad = rad     # 50 km search radius
